...
import type { TableFilterType, TaskResponse } from '@sis-thesqd/mmq-component';
...
interface ProjectsClientProps {
    initialMMQData: TaskResponse | null;
    serverDefaultAccount: number | null;
}

export function ProjectsClient({ initialMMQData, serverDefaultAccount }: ProjectsClientProps) {
    ...
    // Determine if we can use server-prefetched data
    // Only use if account matches server default (no URL/input override)
    const canUseServerData = accountNumber === serverDefaultAccount && !urlOverride && !inputOverride;
                ...
                <Suspense fallback={<MMQSkeleton splitOutActive={splitOutActive} viewType={selectedView as 'board' | 'table'} />}>
                    {accountNumber ? (
                        <ProjectsContent
                            ...
                            initialData={canUseServerData ? initialMMQData : null}
                        />
                    ) : (
